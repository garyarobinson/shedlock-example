...
(
    ...
    agent_reporting_instance_id VARCHAR(255),
    application_name            VARCHAR(255) NOT NULL,
    application_id              VARCHAR(255) NOT NULL,
    last_read                   TIMESTAMP WITH TIME ZONE
);

-- Optional: Add an index to frequently queried
-- columns for performance
CREATE INDEX idx_application_version_organization_id ON
    app_version (organization_id);
CREATE INDEX idx_application_version_application_id ON
    app_version (application_id);
CREATE INDEX idx_application_version_session_id ON app_version
    (session_id);
...
VALUES
    ...
    ('org-iota', '{"campaign_id": "summer_sale_2025"}', 'production', 'session-yz0-009', 'agent-prod-srv1-009', 'Marketing Analytics', 'app-ma-009', NOW() - INTERVAL '90 days'),
    ('org-kappa', '{"user_role": "admin"}', 'staging', 'session-123-010', 'agent-stag-srv2-010', 'Admin Console', 'app-ac-010', NOW() - INTERVAL '200 days');
